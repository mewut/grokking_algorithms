...
// Конструктор HashTable принимает объект obj в качестве параметра и инициализирует свойство _items как объект, в который копируются все пары ключ-значение из obj. Свойство length отвечает за количество пар ключ-значение в хеш-таблице.

// Методы хеш-таблицы:

// set(key, value) - добавляет пару ключ-значение в хеш-таблицу. Если ключ уже существует, то значение обновляется, и метод возвращает предыдущее значение. Если ключа не существовало, то метод возвращает undefined.
// get(key) - возвращает значение по ключу. Если ключа не существует, то возвращает undefined.
// has(key) - проверяет, существует ли ключ в хеш-таблице.
// remove(key) - удаляет пару ключ-значение из хеш-таблицы. Если ключа не существовало, то метод возвращает undefined.
// getKeys() - возвращает массив всех ключей в хеш-таблице.
// getValues() - возвращает массив всех значений в хеш-таблице.
// each(callback) - вызывает функцию callback для каждой пары ключ-значение в хеш-таблице.
// clear() - удаляет все пары ключ-значение из хеш-таблицы.

// Свойства хеш-таблицы:

// length - возвращает количество пар ключ-значение в хеш-таблице.
